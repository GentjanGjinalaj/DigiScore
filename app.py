...
async def index():
    if request.method == 'POST':
        ...
        await asyncio.gather(data_from_pics_task, competitors_from_pics_task, channel_distrib_task, keywords_number_task)

        #getDataFromPics_time = GetDataFromPics.getDataFromPics()

        result = simiweb_task.result() + instagram_task.result() + twitter_task.result() + linkedin_task.result() + facebook_task.result() + links_time + username_time + data_from_pics_task.result() + competitors_from_pics_task.result() + channel_distrib_task.result() + keywords_number_task.result()
